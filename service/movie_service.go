...
// 软删除
func (s *MovieService) DeleteById(id int, isAdmin bool) error {
	// 参数验证
	entity := models.Movie{Id: id}
	rules := govalidator.MapData{
		"id": []string{"required"},
	...
	if err != nil {
		tx.Rollback()
		log.Error("查询视频失败 id="+strconv.Itoa(id), err)
		return common.NewError(-2, "查询失败")
	}
	if !exists {
		tx.Rollback()
	...
	if err := repo.Update(id, data); err != nil {
		tx.Rollback()
		log.Error("视频删除失败 id="+strconv.Itoa(id), err)
...
// 变更状态
func (s *MovieService) ChangeStatus(id int, status int, isAdmin bool) error {
	// 参数验证
	entity := models.Movie{Id: id}
	rules := govalidator.MapData{
		"id": []string{"required"},
	...
	if err := repo.Update(id, data); err != nil {
		tx.Rollback()
		log.Error("修改视频状态失败 id="+strconv.Itoa(id), err)
	...
	// 后台操作日志
	if isAdmin {
		log.Infof("修改视频状态 id=%d status=%d", id, status)
	}
